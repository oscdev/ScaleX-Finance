/**
 * Canonical loan-type codes stored in DB / APIs.
 * UI may still show full labels via loanTypeLabel().
 * normalizeLoanTypeCode() accepts legacy phrases and codes.
 */
...
/** Map legacy / alternate display strings → code. */
...
/**
 * Normalize any known loan-type string to a canonical code.
 * Returns null when input is empty or unrecognized.
 */
export function normalizeLoanTypeCode(
  ...
  // Loose legacy matchers (e.g. "My Business Loan Product")
...
/** Normalize or fall back to DEFAULT_LOAN_TYPE (PL). */
