import { BlScoreErr } from '../utils/error-codes';
import type { LenderScoreResult, RankResult, RankedLender } from '../utils/types';
import { MIN_DISPLAY_SCORE } from '../utils/types';
import type { ScoringRunLogger } from '../../personal-loan-scoring-criteria/utils/scoring-file-logger';
...
export function rankForAiMatch(
  ...
  const belowThreshold: RankedLender[] = eligible
    ...
    .map((l) => ({
      ...
      rank: null,
      displayed: false,
      errorCode: BlScoreErr.BELOW_DISPLAY_THRESHOLD,
    }));
...
export async function rankScoredLenders(
  ...
  if (opts?.fileLog) {
    opts.fileLog.writeRankSummary(rank as any);
    opts.fileLog.writeRunDone(scored.length, rank.displayed.length);
  }

  await logActivity(strapi, {
    action: 'BL_SCORE_RANK_COMPLETE',
    description: `BL rank complete for lead ${leadId}`,
    severity: 'info',
  ...
  await logActivity(strapi, {
    action: 'BL_SCORE_RUN_DONE',
    description: `BL scoring done for lead ${leadId}`,
    severity: 'info',
    model: 'business-loan-scoring-criteria',
    metadata: {
      leadId,
