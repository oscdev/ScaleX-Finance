...
| Automation Testing Suite | [`Automation-Testing/`](Automation-Testing/) — offline PL/BL fixtures + **Live Run** (no files = one `[SUITE-TEST]` lead from `documents/default/{product}/sample-default-*.csv` named scenarios PL-01..25 / BL-01..25; `cibil` filenames resolve from product folder or shared [`documents/default/CIBIL/`](Automation-Testing/documents/default/CIBIL/) Normal + Policy Bazaar samples; CSV + Documents together = max 5 rows; CSV overwrite `documents/upload/{product}/live-run.csv`; PDFs persist under `public/uploads/api_uploads/{leadId}-{name}/`; **Business Loan CSV `turnover` = full ₹**, converted to Lakh in `form_data`; typical BL amount/turnover sized above PL from `lenders_criteria_bl`; suite report / Journey Demo show full ₹) + Journey Demo (eligibility **Show** filter matches Live Run `report.html`: All / Eligible only / Failed only) / Search; offline `pipeline-defs.js` lists full PL 19 + BL **20** steps (incl. `BL-CC-UTIL` no-CC SKIP formula); fixtures/Live Run use live `evaluateCcu`; dashboard at **`/suite`** starts with Strapi (`npm run start` / `npm run dev` → `:4100`; Next rewrite). After server pull restart **Strapi and** Next. See [`Automation-Testing/README.md`](Automation-Testing/README.md) |
...
- **[src/api/system-events/](src/api/system-events/)** — Activity logs + Notifications (admin bell) + Email; UID `api::system-events.activity-log`; thin `services/activity-log.ts` → `activity/`, `notifications/` (`bell-dedupe`, `role-scope`, `list-for-bell`), `email/` (`outbound-send`, `email-audit`, `templates/<event>.html`); bell via `GET /admin/activity-logs/notifications` (Admin = all; Advisor/Staff/Banker = lead association); `logEvent` / `logEventDeduped`; `onLoanApplicationCreated` → Super Admins + advisor only (no applicant); Lead View assign Advisor/Parent → `onLeadAdvisorAssigned`; Assign Staff/Banker → `onLoanStaffBankerAssigned` (same `loan-application.html`); Lead status change → `onLeadStatusChanged` (`lead-status-update.html` to every assigned Advisor/Parent/Staff/Banker); advisor form submit → `onAdvisorRegistrationSubmitted` (`advisor-registration.html` to applicant + Super Admins); advisor Approved or admin Invite user → `onRegistrationWelcome` (`registration-welcome.html`); admin forgot-password HTML from same folder via [`config/admin.ts`](config/admin.ts) `forgotPassword.emailTemplate` (Strapi path — not gated by `emailsIsEnabled`); SMTP [`config/plugins.ts`](config/plugins.ts); gated by `emailsIsEnabled` (custom sends); Activity Email tab = event sub-tabs + collapsible lead groups + `skipReason` (no severity filter); Email person lines map `metadata.role` (`admin`/`advisor`/`parent_advisor`/`staff`/`banker`) to Super Admin / Advisor / Parent Advisor / Staff / Banker (not bare “Recipient”); `loan-application.html` uses nested fixed-width tables for client forward compatibility; coverage in [docs/Activity-Log.md](docs/Activity-Log.md) and [docs/Email-Templates-Guide.md](docs/Email-Templates-Guide.md)
